	...
	while( ev_num<=ev ){ //entering the main while loop and looping over all the events in the Iris tree
		...
		if(det->TYuADC.size()>0){
			...
			for ( unsigned int adci=0; adci < det->TYuADC.size(); adci++ ){
				...
				//cout << adci << "	" << det->TYuADC.at ( adci )<< endl;
			}
		...
		if ( det->TSd1rADC.size() >0 ){
			...
			for ( unsigned int adci=0; adci < det->TSd1rADC.size(); adci++ ){
				if ( det->TSd1rADC.at ( adci ) >0 ){
					Sd1rEnergyRaw.push_back(det->TSd1rADC.at(adci));
			...
			for(size_t i = 0; i < Sd1rEnergyRaw.size(); i++) {
			Sd1rEnergy.push_back((Sd1rEnergyRaw[i]*Sd1rp0[Sd1rChannel[i]] + Sd1rp1[Sd1rChannel[i]])/1000);
			}
		}
		...
		if ( det->TSd1sADC.size() >0 ){
			...
			for ( unsigned int adci=0; adci < det->TSd1sADC.size(); adci++ ){
				if ( det->TSd1sADC.at ( adci ) >0 ){
					Sd1sEnergyRaw.push_back(det->TSd1sADC.at(adci));
			...
			for(size_t i = 0; i < Sd1sEnergyRaw.size(); i++) {
			Sd1sEnergy.push_back((Sd1sEnergyRaw[i]*Sd1sp0[Sd1sChannel[i]] + Sd1sp1[Sd1sChannel[i]])/1000);
		...
		if ( det->TSd2rADC.size() >0 ){
			...
			for ( unsigned int adci=0; adci < det->TSd2rADC.size(); adci++ ){
				if ( det->TSd2rADC.at ( adci ) >0 ){
					Sd2rEnergyRaw.push_back(det->TSd2rADC.at(adci));
			...
			for(size_t i = 0; i < Sd2rEnergyRaw.size(); i++) {
			Sd2rEnergy.push_back((Sd2rEnergyRaw[i]*Sd2rp0[Sd2rChannel[i]] + Sd2rp1[Sd2rChannel[i]])/1000);
		...
		if ( det->TSd2sADC.size() >0 ){
			...
			for ( unsigned int adci=0; adci < det->TSd2sADC.size(); adci++ ){
				if ( det->TSd2sADC.at ( adci ) >0 ){
					Sd2sEnergyRaw.push_back(det->TSd2sADC.at(adci));
			...
			for(size_t i = 0; i < Sd2sEnergyRaw.size(); i++) {
			Sd2sEnergy.push_back((Sd2sEnergyRaw[i]*Sd2sp0[Sd2sChannel[i]] + Sd2sp1[Sd2sChannel[i]])/1000);
