...
double readAlphaYu(){
	...
	chain->Add ( "/home/jerome/12Be_exp/Processed_files/rootindpndt_5225_1.root" );
	chain->Add ( "/home/jerome/12Be_exp/Processed_files/rootindpndt_5225_2.root" );
	chain->Add ( "/home/jerome/12Be_exp/Processed_files/rootindpndt_5225_3.root" );
	...
	while ( ev_num<=ev ){
		...
		if ( YdEnergyRaw->size() >0 && YdMul==1 ){
			if ( YdEnergyRaw->at(0)>0 ){
		...
		if ( CsI1EnergyRaw->size() >0 && CsI1Mul==1 ){
			if ( CsI1EnergyRaw->at ( 0 ) >0 ){
				hCsI1[CsI1Channel->at ( 0 )]->Fill ( CsI1EnergyRaw->at ( 0 ) );
			}
		}
		
		if ( CsI2EnergyRaw->size() >0 && CsI2Mul==1 ){
			if ( CsI2EnergyRaw->at ( 0 ) >0 ){
				hCsI2[CsI2Channel->at ( 0 )]->Fill ( CsI2EnergyRaw->at ( 0 ) );
			}
		}
		
		if ( Sd1rEnergyRaw->size() >0 && Sd1rMul==1 ){
			if ( Sd1rEnergyRaw->at ( 0 ) >500 ){
				hSd1r[Sd1rChannel->at ( 0 )]->Fill ( Sd1rEnergyRaw->at ( 0 ) );
			}
		}
		
		if ( Sd1sEnergyRaw->size() >0 && Sd1sMul==1 ){
			if ( Sd1sEnergyRaw->at ( 0 ) >500 ){
				hSd1s[Sd1sChannel->at ( 0 )]->Fill ( Sd1sEnergyRaw->at ( 0 ) );
			}
		}
		
		if ( Sd2rEnergyRaw->size() >0 && Sd2rMul==1 ){
	...
	for ( int j=0; j<128; j++ ){ //loop over all the strips of the YY detectors
		nfound = s->Search ( hYu[j],2,"",0.1 ); //change the name of the histogram acordingly
		
		for ( int p=0; p<nfound; p++ ){
			xp[p] = txp[p];
		} //loop over the found peaks and fill in the xp variable
		...
		if(xp[0]!=a[j] && xp[0]!=c[j]){
			b[j] = xp[0];
		}else if(xp[1]!=a[j] && xp[1]!=c[j]){
			b[j] = xp[1];
		}else if(xp[2]!=a[j] && xp[2]!=c[j]){
			b[j] = xp[2];
		}
	...
	ofstream YY_alpha;
	YY_alpha.open ( "Yu_AlphaPeaks.txt" ); //change the path and the name of the file accordingly; these are for the Yd and Yu detectors
	YY_alpha << " Strip / Peak 1 / 2 / 3  \n";

	//open a file to store the amplitude of the peaks
	ofstream YY_alphaCounts;
	YY_alphaCounts.open ( "YuCounts.txt" ); //change the path and the name of the file acordingly; these are for Yd and Yu detectors
	...
	for ( int i=0; i<128; i++ ){ //loop over all the Yu/Yd strips to fit the data
		...
		hYu[i]->Fit ( "fit_func6","","",a[i]-100,c[i]+100 ); //fit the desired histograme; change the name to Yu or Yd depending on interest
