...
double analysis()
{
    ...
    TFile *f_out = new TFile("/home/jerome/12Be_exp/scripts/scripts/carbon12_test.root","RECREATE");
    //TFile *f_out = new TFile("Yd_calibration_check.root","RECREATE");
...
//Open the input files
    TChain *chain = new TChain ( "AutoTree" );
     ...
     //test for alphas
    //Check_5225
    /*chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5001.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5002.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5003.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5004.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5006.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5007.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/rootindpndt_5009.root");*/
    ...
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5001.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5002.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5003.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5004.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5006.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5007.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt5225_loss_test_5009.root");
    
    
    /*chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_pedestal_4972.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_pedestal_4973.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_pedestal_4974.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_pedestal_4975.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_pedestal_4977.root");*/
    
    /*chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_4972.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_4973.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_4974.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_4975.root");
    chain->Add("/home/jerome/12Be_exp/Processed_files/Yd_calibration/rootindpndt_4977.root");*/
 ...
 /* chain->Add ( "/mnt/i/12Be_S1506/Processed_rootFiles/NewFiles/Be00_5021.root");
  ...
  chain->Add ( "/mnt/i/12Be_S1506/Processed_rootFiles/NewFiles/Be00_5221.root");*/
    ...
    ifstream geometry;
    geometry.open ( "/home/jerome/12Be_exp/scripts/geometry_s1506.txt" ); //open the calibration file
    if ( !geometry.is_open() )
    {
    ...
    TH2D *hSd1rSd2s = new TH2D("hSd1rSd2s","Sd1r vs Sd2s",2048,0,4096,2048,0,4096); //non calibrated Sd1r vs Sd2s
    TH2D *hSd1sSd2r = new TH2D("hSd1sSd2r","Sd1s vs Sd2r",2048,0,4096,2048,0,4096); //non calibrated Sd1s vs Sd2r
    TH2D *hSd1sSd2s = new TH2D("hSd1sSd2s","Sd1s vs Sd2s",2048,0,4096,2048,0,4096); //non calibrated Sd1s vs Sd2s
    ...
    TH2D *hCSd1rSd2rIC = new TH2D("hCSd1rSd2rIC","Cal Sd1r vs Sd2r, gated by IC",2048,0,4096,2048,0,4096); // calibrated Sd1r vs Sd2r
    TH2D *hCSd1rSd2rICCut = new TH2D("hCSd1rSd2rICCut","Cal Sd1r vs Sd2r, gated by IC gated by the channel corr",2048,0,4096,2048,0,4096); // calibrated Sd1r vs Sd2r
    ...
    TH2D *hCSd1rSd2rYdICCut = new TH2D("hCSd1rSd2rYdICCut","Cal Sd1r vs Sd2r, gated by Yd & IC & chann corr",2048,0,4096,2048,0,4096); // calibrated Sd1r vs Sd2r
    ...
    double Sdbins[25]={0}, Sdbintemp[25]={0};
    for (int i=0; i<25; i++){Sdbintemp[i]=TMath::RadToDeg()*TMath::ATan((Sdr0+(24-i)*((Sdr1-Sdr0)/24))/Sd1z);
    }
    
    for(int k=24; k>=0; k--)
    {
      int a = 24-k;
      Sdbins[a]=Sdbintemp[k];
    }
    
    
    TH2D *hYdAn = new TH2D("hYdAn","YdE vs Angle", 16,Ydbins,1000,0,10); //no gates at the moment
    TH2D *hSd1An = new TH2D("hSd1An","Sd1dE vs Angle", 24,Sdbins,1000,0,10);
    TH2D *hYdAnIC = new TH2D("hYdAnIC","YdE vs Angle with an IC gate", 16,Ydbins,1000,0,5); //gate on the IC peak
    ...
    TH1D *hYuEn = new TH1D("hYuEn","hYuEn",1250,0,10);
    TH1D *hYdEn = new TH1D("hYdEn","hYdEn",1250,0,10);
    TH1D *hSd1En = new TH1D("hSd1En","hSd1En",1250,0,10);
    TH2D *hYdCsI[16];
    TH2D *hYdCsI2[16];
    ...
    TH2D *hYuAn = new TH2D("hYuAn","YuE vs Angle",16,Yubins,1000,0,10); //no gates at the moment
    ...
    TH2D *hYuAnPIDW = new TH2D("hYuAnPIDW","YuE vs Angle with a PID wide gate",16,Yubins,1000,0,10); //gate on the Sds
    TH2D *hSd1r = new TH2D("hSd1r","Sd1r vs Angle",24,Sdbins,1000,0,10);
    ...
    for(int i=0; i<8; i++)
    {
        ...
        hYuES[i] = new TH1D ( name1.c_str(),"Yu energy for sector",5000,0,10 );
      
        string name1yd = Form ( "hYdES_%i",i );
        hYdES[i] = new TH1D ( name1yd.c_str(),"Yd energy for sector",5000,0,10 );
    ...
    for(int i=0; i<16; i++)
    {
        ...
        string name3yd = Form ( "hYdER_%i",i );
        hYdER[i] = new TH1D ( name3yd.c_str(),"Yd energy for ring",1250,0,10 );
    ...
    float SdWidth = (Sdr1-Sdr0)/24;
    float YChMiddle = YChWidth/2;
    float SdMiddle = SdWidth/2;
    float yuM=0, ydM=0, Sd1rM=0;
    double YuthetaM, YdthetaM, Sd1thetaM; //angle for Yu/Yd
    ...
    vector<double> *SdAngle = new vector<double>;
	...
	//if(TMath::Abs(Sd2rChannel->at(0)-Sd1rChannel->at(0))==1){hCSd1rSd2r->Fill(Sd2rEnergy->at(0),Sd1rEnergy->at(0));}
	 if ( ICEnergyRaw->size() >0 ){if ( ICEnergyRaw->at ( 0 ) >1500 && ICEnergyRaw->at ( 0 ) <2300){hCSd1rSd2rIC->Fill ( Sd2rEnergyRaw->at ( 0 ),Sd1rEnergyRaw->at ( 0 ) );}}  
            if ( ICEnergyRaw->size() >0 ){if ( ICEnergyRaw->at ( 0 ) >1500 && ICEnergyRaw->at ( 0 ) <2300 && TMath::Abs(Sd2rChannel->at(0)-Sd1rChannel->at(0))==1){hCSd1rSd2rICCut->Fill ( Sd2rEnergyRaw->at ( 0 ),Sd1rEnergyRaw->at ( 0 ) );}}   //IC gate for C 1500 < E < 200; for Be 600< E < 1100
	      ...
	      if ( YdEnergy->size() >0 && YdMul==1 && ICEnergyRaw->size() >0 && TMath::Abs(Sd2rChannel->at(0)-Sd1rChannel->at(0))==1){ if(YdEnergy->at( 0 ) >0 && ICEnergyRaw->at ( 0 ) >1500 && ICEnergyRaw->at ( 0 ) <2300)
	      {hCSd1rSd2rYdICCut->Fill(Sd2rEnergyRaw->at(0),Sd1rEnergyRaw->at(0));}}
        ...
        if ( YdEnergy->at ( 0 ) >0 && ( YdChannel->at ( 0 ) !=20 && YdChannel->at ( 0 ) !=0 && YdChannel->at ( 0 ) !=55 &&  YdChannel->at ( 0 ) !=127 ) )
            {
               ...
               //hYdES[YdSector->at ( i )]->Fill ( YdEnergy->at ( i ) );
                //hYdER[YdRing->at ( i )]->Fill ( YdEnergy->at ( i ) );
                //hYdEn->Fill(YdEnergy->at ( i ));
		//cout << YdEnergy->at ( i ) << endl;
                //hYdAn->Fill ( YdthetaM,YdEnergy->at ( i ) ); //all, no cuts
               // if(ICEnergyRaw->size()>0) { if(ICEnergyRaw->at(0)>1500 && ICEnergyRaw->at(0)<2300){hYdAnIC->Fill(YdthetaM,YdEnergy->at ( 0 ));}} //IC gate for C 1500 < E < 200; for Be 600< E < 1100
    ...
    if(Sd1rChannel->size()>0  && Sd1sChannel->size()>0 && Sd1rEnergy->size()>0 && Sd1rEnergy->size()==Sd1rChannel->size() && Sd1rChannel->size()==Sd1sChannel->size()) //removed the multiplicity because of alpha test
      {
	//cout << " I am in the loop "<< endl;
	for(unsigned int i=0; i<Sd1rChannel->size(); i++)
	{
       //cout << " I am in the loop "<< endl;
         if (Sd1rEnergy->at ( i ) >0)
	 {
	  // if(YuEnergy->at ( i )>1) {cout << "En " << YuEnergy->at ( i ) << " Channel " << YuChannel->at ( i ) << endl;}
                    Sd1rM = Sdr0+ ( Sd1rChannel->at ( i ) *SdWidth )-SdMiddle;
                    TVector3 SdposM ( 0,Sd1rM,Sd1z );
                    SdposM.SetPhi ( TMath::Pi() /2-Sd1sChannel->at ( i ) *TMath::Pi() /4 ); //Pi/2 because the center of the sector is at 90degrees, Pi/4 is because there is 8 sectors so it is 2Pi/8
                    Sd1thetaM = beam.Angle ( SdposM )*TMath::RadToDeg();
                    SdAngle->push_back(Sd1thetaM);
		    
          // cout << "blah" << endl;        
                 
		    
		   hSd1An->Fill ( Sd1thetaM,Sd1rEnergy->at ( i ) ); //all, no cuts
		   hSd1En->Fill(Sd1rEnergy->at ( i ));
	...
	{
       // cout << " I am in the loop "<< endl;
	 //if (YuEnergy->at ( i ) >0 && (YuChannel->at ( i ) !=82 && YuChannel->at ( i ) !=96 && YuChannel->at ( i ) !=106  ))
         if (YuEnergy->at ( i ) >0 && ( YuChannel->at ( i ) !=80 && YuChannel->at ( i ) !=95 && YuChannel->at ( i ) !=94 && YuChannel->at ( i ) !=82 && YuChannel->at ( i ) !=96 && YuChannel->at ( i ) !=106 && YuChannel->at ( i ) !=111 ))
	 {
	  // if(YuEnergy->at ( i )>1) {cout << "En " << YuEnergy->at ( i ) << " Channel " << YuChannel->at ( i ) << endl;}
		   ...
		   hYuEn->Fill(YuEnergy->at ( i ));
		      ...
		      if ( ICEnergyRaw->at ( 0 ) >600 && ICEnergyRaw->at ( 0 ) <1100 )
                    ...
                    */
                   if(Sd2rEnergyRaw->size()>0 && Sd1rEnergyRaw->size()>0 && ICEnergyRaw->size()>0)
		    {
                      if ( pidcut->IsInside ( Sd2rEnergyRaw->at ( 0 ),Sd1rEnergyRaw->at ( 0 ) ) && ICEnergyRaw->at ( 0 ) >1500 && ICEnergyRaw->at ( 0 ) <2200 )
                    {
    ...
    double xp[3];
    double aS[8],bS[8],cS[8],aR[16], bR[16], cR[16],a,b,c;
    
    
    //YuEn fitting
    
    nfound = s->Search ( hYuEn,2,"",0.1 );
      
            for ( int p=0; p<nfound; p++ ){xp[p] = txp[p];}
          
     a = min ( min (xp[0],xp[1]), xp[2]);
     c = max ( max (xp[0],xp[1]), xp[2]);
    
    if(xp[0]!=a && xp[0]!=c) {b = xp[0];}
    else if(xp[1]!=a && xp[1]!=c) {b = xp[1];}
    else if(xp[2]!=a && xp[2]!=c) {b = xp[2];}
        ...
        fit_func6->SetParLimits ( 1,a-0.05,a-0.075 );
        fit_func6->SetParLimits ( 4,a-0.075,a+0.075 );
        fit_func6->SetParLimits ( 6,b-0.05,b-0.075 );
        fit_func6->SetParLimits ( 8,b-0.075,b+0.075 );
        fit_func6->SetParLimits ( 10,c-0.05,c-0.075 );
        //fit_func6->SetParLimits ( 11,180000,190000 );
        fit_func6->SetParLimits ( 12,c-0.075,c+0.1 );
        fit_func6->SetParLimits ( 2,0.01,0.3 );
	
	 hYuEn->Fit ( "fit_func6","","",a-0.5,c+0.5 );*/
   ...
   /* int npeaks=3;
    ...
    if(xp[0]!=a && xp[0]!=c) {b = xp[0];}
        ...
        fit_func9->SetParLimits ( 1,a-0.15,a-0.11 );
        fit_func9->SetParLimits ( 4,a-0.05,a-0.04 );
        fit_func9->SetParLimits ( 6,a-0.015,a+0.015 );
        fit_func9->SetParLimits ( 8,b-0.15,b-0.11 );
        fit_func9->SetParLimits ( 10,b-0.05,b-0.04 );
        fit_func9->SetParLimits ( 12,b-0.015,b+0.015 );
        fit_func9->SetParLimits ( 14,c-0.15,c-0.11 );
        fit_func9->SetParLimits ( 16,c-0.05,c-0.04 );
        //fit_func9->SetParLimits ( 11,180000,190000 );
        fit_func9->SetParLimits ( 18,c-0.015,c+0.015 );
        fit_func9->SetParLimits ( 2,0.01,0.2 );
	
	 hYdEn->Fit ( "fit_func9","","",a-0.5,c+0.5 );*/
 ...
 //   hQval->Write();
  ...
  //  hCSd1rSd2rICYu->Write();
   ...
   //for(int i=0; i<7; i++){hYdES[i]->Write();}
 //  for(int i=0; i<7; i++){hYuESIC[i]->Write();}
    ...
    //for ( int j=0; j<16; j++ ){hYdER[j]->Write();}
