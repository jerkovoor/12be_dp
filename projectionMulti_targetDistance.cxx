//Calculates and plots the residual vs. the angle for different target distances and do linear fit.
//Also plots the fit parameters vs the target distance and finds the minima for both the slope and the intercept.
...
double kin(double *x, double *par){ 
	...
	float Q_C = Qgs13C;//-Exenergy; //Change accordingly for GS or TES
...
void projectionMulti_targetDistance() {
	...
	// Energies from 2.85 to 4.68 were selected after inspecting all the 16 histograms for the GS
	...
	while (th<=93){
		...
		string f_name=Form("../Analysis/BeamOffset/TargetDistance/C_pedestal_TRIUMF_DL_NoOffset_NoShift_Yu_%imm.root",th);
