...
double analysis(){
	...
	TFile *f_out = new TFile("/home/jerome/12Be_exp/Analysis/TRIUMF_DL/Be_pedestal_50keVBins_TRIUMF_DL.root","RECREATE"); //change the path and name accordingly
	...
	TH1D *hYuEn = new TH1D("hYuEn","hYuEn with energy loss",2500,0,10); //energy singles in Yu
	...
	TH2D *hYuAn = new TH2D("hYuAn","YuE vs Angle with energy loss",16,Yubins,250,0,10); //Energy vs angle in Yu no gates
	...
	TH2D *hYuAnIC = new TH2D("hYuAnIC","YuE vs Angle with a gate on the IC and with energy loss",16,Yubins,240,0,2.4); //Energy vs angle in Yu with a gate on IC
	TH2D *hYuAnICT = new TH2D("hYuAnICT","YuE vs Angle with a gate on the IC and with target energy loss",16,Yubins,240,0,2.4); 
	TH2D *hYuAnIC1 = new TH2D("hYuAnIC1","YuE vs Angle with a gate on the IC",16,Yubins,240,0,2.4);
	TH2D *hYuAnPID = new TH2D("hYuAnPID","YuE vs Angle with an IC and PID gate and with energy loss",16,Yubins,240,0,2.4); //Energy vs angle in Yu with a gate on IC and PID
	TH2D *hYuAnPIDT = new TH2D("hYuAnPIDT","YuE vs Angle with an IC and PID gate and with target energy loss",16,Yubins,240,0,2.4);
	TH2D *hYuAnPID1 = new TH2D("hYuAnPID1","YuE vs Angle with an IC and PID gate",16,Yubins,240,0,2.4);
    
	//Q values
	TH1D *hQval = new TH1D("hQval","Q values with energy loss",400,-10,10);
	...
	while(ev_num<=ev){
		...
		if(Sd1rChannel->size()>0 && Sd2rChannel->size()>0 && Sd1rMul==1 && Sd2rMul==1){
			...
			if(TMath::Abs(Sd2rChannel->at(0)-Sd1rChannel->at(0))<2 && ICEnergyRaw>620 && ICEnergyRaw<1100){hSd1rSd2rCut->Fill(Sd2rEnergyRaw->at(0),Sd1rEnergyRaw->at(0));} //Non-calibrated Sd1r vs Sd2r energy with an ring correlation requirenment and IC gate - PID plot
		}
		...
		if(Sd1rEnergy->size()>0 && Sd2rEnergy->size()>0 && Sd1rMul==1 && Sd2rMul==1){
			...
			if ( ICEnergyRaw>620 && ICEnergyRaw<1100){
				...
				if (TMath::Abs(Sd2rChannel->at(0)-Sd1rChannel->at(0))<2){
			...
			if (YuEnergy->at(0)>0.1 && ( YuChannel->at(0)!=82 && YuChannel->at(0)!=96 && YuChannel->at(0)!=106 && YuChannel->at(0)!=111)){
				...
				for(int RingNumber=0; RingNumber<16; RingNumber++){	    
					for(int j=0; j<8; j++){ 
						...
						if(YuChannel->at(0)==j*16+RingNumber){
							...
							hYuEnT->Fill(YuEnergy->at(0)+loss3);
							hYuAn->Fill(YuthetaM,YuEnergy->at(0)+loss);
							hYuAnT->Fill(YuthetaM,YuEnergy->at(0)+loss3);
							if(ICEnergyRaw>620 && ICEnergyRaw<1100){
								hYuAnIC->Fill(YuthetaM,YuEnergy->at (0)+loss);
								hYuAnICT->Fill(YuthetaM,YuEnergy->at (0)+loss3);
							...
							if(Sd2rEnergy->size()>0 && Sd1rEnergy->size()>0){
								if(pidcut->IsInside(Sd2rEnergy->at(0),Sd1rEnergy->at(0)) && ICEnergyRaw>620 && ICEnergyRaw<1100 ){
									...
									hYuAnPIDT->Fill(YuthetaM,YuEnergy->at(0)+loss3);
									...
									QvalT = ( 1+mejec/mrecoil ) * ( YuEnergy->at(0)+loss3) - ( 1 - mbeam/mrecoil ) * ( kBeam ) - 2 * TMath::Sqrt ( mbeam*mejec* ( YuEnergy->at(0)+loss3) * ( kBeam ) ) / mrecoil * TMath::Cos ( YuthetaM * TMath::Pi() / 180. );
									hQvalT->Fill ( QvalT);
				...
				if(Sd2rEnergy->size()>0 && Sd1rEnergy->size()>0){
					if (pidcut->IsInside(Sd2rEnergy->at ( 0 ),Sd1rEnergy->at ( 0 ) ) && ICEnergyRaw>620 && ICEnergyRaw<1100 ){
						...
						Qval1 = ( 1+mejec/mrecoil ) * ( YuEnergy->at(0) ) - ( 1 - mbeam/mrecoil ) * ( kBeam ) - 2 * TMath::Sqrt ( mbeam*mejec* ( YuEnergy->at(0) ) * ( kBeam ) ) / mrecoil * TMath::Cos ( YuthetaM * TMath::Pi() / 180. );
						hQval1->Fill ( Qval1 );
					}
				}
			}
