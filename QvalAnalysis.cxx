...
double QvalAnalysis(){
	...
	TFile *f_out = new TFile("/home/jerome/12Be_exp/Analysis/CarbonGain/C_pedestal_TRIUMF_DL_CarbonGain_NewDecode_NewCut_Yu_Random.root","RECREATE"); //change the path and name accordingly
	...
	for(int i=0;i<shiftLength;i++){
		...
		hQval[i] = new TH1D(namehQval.c_str(),"Q values",400,-10,10);//240,0,2.4
				...
				for(int shiftNumber=0;shiftNumber<shiftLength;shiftNumber++){
					if(Sd2rDetector.size()>0 && Sd1rDetector.size()>0){
						...
						//if(pidcut->IsInside(Sd2rDetector[0].energy,Sd1rDetector[0].energy) && ICEnergyRaw>620 && ICEnergyRaw<1100 ){
							hYuAnPID[shiftNumber]->Fill(YuthetaR,YuDetector[0].energy+loss+shift[shiftNumber]);
							Qval = ( 1+mejec/mrecoil ) * ( YuDetector[0].energy+loss+shift[shiftNumber]) - ( 1 - mbeam/mrecoil ) * ( kBeam ) - 2 * TMath::Sqrt ( mbeam*mejec* ( YuDetector[0].energy+loss+shift[shiftNumber]) * ( kBeam ) ) / mrecoil * TMath::Cos ( YuthetaR * TMath::Pi() / 180. );
